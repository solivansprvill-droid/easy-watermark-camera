...
public final class Watermark {
    ...
    public static final int TEMPLATE_MARK = 0;   // 马克式（左下信息块）
    public static final int TEMPLATE_MINI = 1;   // 极简式（两行小字）
    public static final int TEMPLATE_BAR  = 2;   // 通栏式（底部全宽条）
    ...
    private static void drawMark(Bitmap bmp, Date when, String code, Params p) {
        ...
        float blockW = blockWidth(timeP, dateP, fieldP, verifyP, antiP, noteP,
                t1, t2, tAddr, tGeo, tNote, tVerify(p), tAnti);

        int A = Math.round(255 * p.alpha);
        int textWhite = (A << 24) | 0xFFFFFF;
        int textDim = (Math.round(255 * p.alpha * 0.85f) << 24) | 0xFFFFFF;
        ...
        float x = marginX;
        float y = bmp.getHeight() - marginX - totalH;
        if (y < marginX) y = marginX;

        if (p.logo != null) {
            Paint lp = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.FILTER_BITMAP_FLAG);
            lp.setAlpha(A);
            c.drawBitmap(p.logo, null, new RectF(x, y, x + logoW, y + logoH), lp);
            y += logoH + gap * 0.6f;
        }

        timeP.setColor(textWhite); shadow(timeP, p.alpha);
        c.drawText(t1, x, y + hTime, timeP);
        y += hTime + gap * 0.4f;
        dateP.setColor(textDim); shadow(dateP, p.alpha);
        c.drawText(t2, x, y + hDate, dateP);
        y += hDate + gap;
        if (tAddr != null) { fieldP.setColor(textWhite); shadow(fieldP, p.alpha); c.drawText(tAddr, x, y + hField, fieldP); y += hField + gap * 0.6f; }
        if (tGeo != null) { fieldP.setColor(textDim); c.drawText(tGeo, x, y + hField, fieldP); y += hField + gap * 0.6f; }
        if (tNote != null) { noteP.setColor(textWhite); shadow(noteP, p.alpha); c.drawText(tNote, x, y + hNote, noteP); y += hNote + gap * 0.6f; }
        if (tVerify != null) { verifyP.setColor(textWhite); shadow(verifyP, p.alpha); c.drawText(tVerify, x, y + hVerify, verifyP); y += hVerify + gap * 0.5f; }
    ...
    private static void drawMini(Bitmap bmp, Date when, String code, Params p) {
        ...
        float x = marginX, y = bmp.getHeight() - marginX - totalH;
        if (y < marginX) y = marginX;

        lineP.setColor(textWhite); shadow(lineP, p.alpha);
        c.drawText(l1, x, y + hLine, lineP);
        lineP.setColor(textDim);
        c.drawText(l2s, x, y + hLine + gap + hLine, lineP);
    }
    ...
    private static void drawBar(Bitmap bmp, Date when, String code, Params p) {
        ...
        float barTop = bmp.getHeight() - barH;

        int A = Math.round(255 * p.alpha);
        Paint bg = new Paint();
        bg.setColor((Math.round(255 * p.alpha * 0.62f) << 24));
        c.drawRect(0, barTop, bmp.getWidth(), bmp.getHeight(), bg);

        int textWhite = (A << 24) | 0xFFFFFF;
        int textDim = (Math.round(255 * p.alpha * 0.82f) << 24) | 0xFFFFFF;
