...
- 🖼 **相册（v1.5）**：主界面快门左侧新增「相册」入口，浏览本 App 保存的无水印原图，点选可 **编辑（裁剪/滤镜/加字）后重新合成水印**、分享、删除
- 📍 **地址保留（v1.7 修复）**：拍摄时地址写入 EXIF（ImageDescription），相册编辑/批量补印时自动恢复原地址；无存储地址时由 EXIF GPS 实时反查（Geocoder/Nominatim）；手动选址始终最优先
- 🌐 **全部照片（v1.5）**：右上角新增「全部照片」入口，浏览设备全部照片，任选一张打上水印并进入编辑，兼做批量导入补充
- 📍 **手动选址（v1.6）**：相机屏「📍选址」按钮与设置面板均可手动输入固定地址并长期保存，水印优先使用；清除后恢复 GPS 自动定位，相册补印同样生效
